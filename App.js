...
import { primaryDark, primaryLight, primary, white } from './utils/colors'
...
const RouteConfigs = {
  ...
  NewDeck:{
    component: NewDeck,
    name: "Add Deck",
    options: {tabBarIcon: ({color}) => <AntDesign name="pluscircleo" size={ 20 } color={ color }/>, title: 'Add Deck'}
  }
}
  ...
  return (
    <Stack.Navigator screenOptions={{
      ...
      <Stack.Screen name="NewCard" 
        component={NewCard} 
        options={({ route }) => ({ 
          title: `Add Card To ${route.params.title}` 
        })} 
      />
...
export default function App() {
  return (
    <Provider store={store}>
      <View style={{flex: 1}}>
        <TheStatusBar backgroundColor={primaryDark} barStyle="light-content" />
        <NavigationContainer>
          <MainNav />
        </NavigationContainer>
      </View>
